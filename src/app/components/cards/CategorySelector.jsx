...
export function validateCategory (category, required = true) {
  ...
  // !category || category.trim() === '' ? 'Required' :
    cats.length > 10
      ...
      : cats.filter((c) => c.substring(0, 6) === 'blurt-').length > 1
      ? tt('category_selector_jsx.must_not_include_nexus_community_owner', {
          blurt: cats.filter((c) => c.substring(0, 6) === 'blurt-')[0],
        })
      : null
  )
}
